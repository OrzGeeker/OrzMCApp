...
- Release App：复用 `scripts/release-macos.sh` 构建、Developer ID 签名、公证、生成 appcast、创建或更新 GitHub Release、上传 ZIP/DMG，并提交 `products/appcast.xml`。
...
- 脚本会生成 Sparkle ZIP、签名 DMG、`appcast.xml`，并创建或复用同版本 tag 的 Release。
...
- 运行 Publish Docs，生成 DocC 文档并发布到 `gh-pages`。
...
- 项目站点根路径已生成 `index.html` 重定向到文档首页。
